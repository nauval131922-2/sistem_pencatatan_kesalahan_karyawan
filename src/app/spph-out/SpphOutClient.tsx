...
export default function SpphOutClient() {
  ...
  useEffect(() => {
    ...
    const saved = localStorage.getItem('spphOutState');
    if (saved) {
      try {
      ...
      if (successCount > 0) {
        ...
        await fetch('/api/activity-log', {
          ...
          body: JSON.stringify({
            action_type: 'SCRAPE',
            table_name: 'spph_out',
            message: `Tarik Data SPPH Out (${formatDateToYYYYMMDD(startDate)} s/d ${formatDateToYYYYMMDD(endDate)})`,
        ...
        setDialog({
          ...
          type: 'success',
          title: 'Berhasil',
          message: `Berhasil menarik ${totalScraped} data SPPH Out.`
        });
      }
    } catch (err: any) {
      ...
      {/* Results View */}
        ...
        <div className="flex flex-col gap-4 shrink-0">
          ...
          <div className="relative w-full group">
            ...
            <input
              type="text"
              placeholder="Cari faktur, supplier, PR..."
            ...
            <>
                    ...
                    <thead className="sticky top-0 z-20 bg-white shadow-sm">
                      <tr>
                        ...
                        <th className="px-6 py-3 border-b border-r border-gray-200 relative group cursor-pointer hover:bg-gray-50 transition-colors whitespace-nowrap overflow-hidden" style={{ width: columnWidths.faktur }} onClick={() => toggleSort('faktur')}>
                          <div className="flex items-center gap-2 text-[12px] leading-none text-[#6b7280] font-bold tracking-wider overflow-hidden">
                            <span className="truncate">Faktur SPPH</span>
                        ...
                        <th className="px-6 py-3 border-b border-r border-gray-200 relative group cursor-pointer hover:bg-gray-50 transition-colors whitespace-nowrap overflow-hidden" style={{ width: columnWidths.faktur_pr }} onClick={() => toggleSort('faktur_pr')}>
                          <div className="flex items-center gap-2 text-[12px] leading-none text-[#6b7280] font-bold tracking-wider overflow-hidden">
                            <span className="truncate">Faktur PR</span>
                            <SortIcon config={sortConfig} sortKey="faktur_pr" />
                          </div>
                        ...
                        <th className="px-6 py-3 border-b border-r border-gray-200 relative group cursor-pointer hover:bg-gray-50 transition-colors whitespace-nowrap overflow-hidden" style={{ width: columnWidths.kd_supplier }} onClick={() => toggleSort('kd_supplier')}>
                          <div className="flex items-center gap-2 text-[12px] leading-none text-[#6b7280] font-bold tracking-wider overflow-hidden">
                            ...
                            <SortIcon config={sortConfig} sortKey="kd_supplier" />
                          </div>
                        ...
                        <th className="px-6 py-3 border-b border-r border-gray-200 relative group cursor-pointer hover:bg-gray-50 transition-colors whitespace-nowrap overflow-hidden" style={{ width: columnWidths.keterangan }} onClick={() => toggleSort('keterangan')}>
                          <div className="flex items-center gap-2 text-[12px] leading-none text-[#6b7280] font-bold tracking-wider overflow-hidden">
                            <span className="truncate">Keterangan</span>
                            <SortIcon config={sortConfig} sortKey="keterangan" />
                          </div>
                        ...
                        <th className="px-6 py-3 border-b border-gray-200 relative group cursor-pointer hover:bg-gray-50 transition-colors whitespace-nowrap overflow-hidden" style={{ width: columnWidths.faktur_sph }} onClick={() => toggleSort('faktur_sph')}>
                          <div className="flex items-center gap-2 text-[12px] leading-none text-[#6b7280] font-bold tracking-wider overflow-hidden">
                            <span className="truncate">Status SPH</span>
                      ...
                      {paginatedData.map((row, idx) => {
                        ...
                        >
                          <td className="p-0 border-b border-gray-200 whitespace-nowrap overflow-hidden">
                             <div className={`px-6 py-1 border-r border-gray-200 h-fit flex items-center text-[12px] leading-none font-bold transition-colors ${isSelected ? 'text-green-700' : 'text-gray-700'}`}>
                               {formatIndoDateStr(row.tgl)}
                             </div>
                          </td>
                          <td className="p-0 border-b border-gray-200 whitespace-nowrap overflow-hidden">
                            <div className={`px-6 py-1 border-r border-gray-200 h-fit flex items-center text-[12px] leading-none font-bold transition-colors ${isSelected ? 'text-green-600' : 'text-gray-700'}`}>
                              {row.faktur}
                            </div>
                          </td>
                          <td className="p-0 border-b border-gray-200 whitespace-nowrap overflow-hidden">
                            <div className={`px-6 py-1 border-r border-gray-200 h-fit flex items-center text-[12px] leading-none font-bold transition-colors ${isSelected ? 'text-green-600' : 'text-gray-500'}`}>
                              {row.faktur_pr}
                            </div>
                          </td>
                          <td className="p-0 border-b border-gray-200 whitespace-nowrap overflow-hidden">
                            <div className={`px-6 py-1 border-r border-gray-200 h-fit flex items-center text-[12px] leading-none font-bold truncate transition-colors ${isSelected ? 'text-green-800' : 'text-gray-700'}`}>
                              {row.kd_supplier}
                            </div>
                          </td>
                          <td className="p-0 border-b border-gray-200 whitespace-nowrap overflow-hidden">
                            <div className={`px-6 py-1 border-r border-gray-200 h-fit flex items-center text-[12px] leading-none font-bold truncate transition-colors ${isSelected ? 'text-green-800' : 'text-gray-700'}`} title={row.keterangan || ''}>
                              {row.keterangan || '–'}
                            </div>
                          </td>
                          <td className="p-0 border-b border-gray-200 whitespace-nowrap overflow-hidden">
                            <div className={`px-6 py-1 h-fit flex items-center text-[12px] leading-none font-bold whitespace-nowrap truncate ${isSelected ? 'text-green-600' : 'text-gray-400'}`}>
                              <span className="flex items-center gap-1.5 leading-none">{row.faktur_sph?.replace(/<[^>]*>/g, '').trim() || '-'}</span>
                            </div>
                          </td>
                        </tr>
                      )})}
              ...
              <div className="flex items-center justify-between shrink-0 px-1 mt-1">
                <span className="text-[12px] leading-none font-bold text-gray-400">
                  {totalCount === 0 ? 'Tidak ada data SPPH Out' : `Menampilkan ${paginatedData.length} dari ${totalCount} data SPPH Out`}
                </span>
                <div className="flex items-center gap-4">
                  {selectedIds.size > 0 && (
