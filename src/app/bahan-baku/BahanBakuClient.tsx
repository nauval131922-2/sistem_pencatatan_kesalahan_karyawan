...
export default function BahanBakuClient() {
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col gap-5 animate-in fade-in duration-500 overflow-hidden">
      ...
      {data && data.length > 0 && (
        <div className="flex-1 bg-white border border-gray-200 shadow-sm rounded-[10px] overflow-hidden flex flex-col min-h-0">
            ...
            >
              <thead className="sticky top-0 z-10 bg-gray-50 border-b border-gray-100">
                <tr className="text-[13px] text-gray-400 font-bold uppercase tracking-wider">
                  ...
                  >
                    <div className="flex items-center justify-end gap-2 nowrap overflow-hidden">HARGA <SortIcon config={sortConfig} sortKey="hp" /></div>
                    <div 
                      className="absolute -right-2 top-0 bottom-0 w-4 z-30 cursor-col-resize group/resizer"
                  ...
                  >
                    <div className="flex items-center gap-2 nowrap overflow-hidden">PRODUK <SortIcon config={sortConfig} sortKey="nama_prd" /></div>
                    <div 
                      className="absolute -right-2 top-0 bottom-0 w-4 z-20 cursor-col-resize group/resizer"
                      onMouseDown={(e) => startResizing('nama_prd', e)}
                      onClick={(e) => e.stopPropagation()}
                    >
                      <div className="absolute inset-y-0 right-2 w-[1px] bg-gray-200 group-hover:bg-green-500/50 transition-colors" />
                    </div>
                  </th>
                </tr>
              </thead>
                  ...
                  return (
                    <tr 
                      ...
                      className={`transition-all duration-150 group h-11 cursor-pointer select-none border-b border-gray-100 ${
                        isSelected ? 'bg-green-50 shadow-[inset_4px_0_0_0_#16a34a]' : isOdd ? 'bg-slate-50/40' : 'bg-white'
                    ...
                    >
                      ...
                      <td className={`px-5 py-1 border-r border-gray-100 text-[13px] font-bold transition-colors nowrap overflow-hidden ${isSelected ? 'text-green-600' : 'text-gray-400'}`}>
                        {row.faktur}
                      </td>
                      <td className={`px-5 py-1 border-r border-gray-100 text-[13px] font-bold transition-colors nowrap overflow-hidden ${isSelected ? 'text-green-800' : 'text-gray-700'}`}>
                        {row.nama_barang}
                      </td>
                      <td className={`px-5 py-1 text-right border-r border-gray-100 text-[13px] font-extrabold transition-colors nowrap overflow-hidden ${isSelected ? 'text-green-700' : 'text-gray-800'}`}>
                      ...
                      <td className={`px-5 py-1 text-right border-r border-gray-100 text-[13px] font-medium transition-colors nowrap overflow-hidden ${isSelected ? 'text-green-600/60' : 'text-gray-400'}`}>
                        {row.hp ? row.hp.toLocaleString('id-ID') : '-'}
                      </td>
                      <td className={`px-5 py-1 text-[13px] font-bold transition-colors nowrap overflow-hidden ${isSelected ? 'text-green-900' : 'text-gray-800'}`}>
                        {row.nama_prd || '-'}
                      </td>
                    </tr>
                  );
