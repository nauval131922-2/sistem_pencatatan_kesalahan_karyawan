...
export default function PelunasanPiutangClient() {
  ...
  useEffect(() => {
    ...
    const savedPeriod = localStorage.getItem('PelunasanPiutangClient_scrapedPeriod');
    if (savedPeriod) {
      try {
    ...
    const saved = localStorage.getItem('ppState');
    if (saved) {
      try {
  ...
  const handleFetch = async () => {
    ...
    try {
      ...
      if (successCount > 0) {
        ...
        localStorage.setItem('PelunasanPiutangClient_scrapedPeriod', JSON.stringify(periodStr));
