...
export default function SphInClient() {
  ...
  const handleFetch = async () => {
    ...
    const processChunk = async (chunk: any) => {
      try {
        const res = await fetch(`/api/scrape-sph-in?start=${chunk.start}&end=${chunk.end}&metaStart=${startStr}&metaEnd=${endStr}`);
        if (res.ok) {
          successCount++;
          const json = await res.json();
    ...
    try {
      ...
      if (successCount > 0) {
        const periodStr = persistScraperPeriod({ stateKey: 'sphInState', periodKey: 'SphInClient_scrapedPeriod' }, startDate, endDate);
  ...
  const columns: ColumnDef<any>[] = useMemo(() => [
    ...
    {
      accessorKey: 'spph_edited_at',
      header: 'SPPH Edited At',
      size: columnWidths.spph_edited_at,
      cell: ({ getValue }) => <span className="text-[11px] text-gray-400 font-mono">{getValue() as string || '-'}</span>
    },
    {
