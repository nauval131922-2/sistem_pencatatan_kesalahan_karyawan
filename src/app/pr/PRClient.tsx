...
export default function PRClient() {
  ...
  const columns: ColumnDef<any>[] = useMemo(() => [
    ...
    {
      accessorKey: 'tgl_dibutuhkan',
      header: 'Dibutuhkan',
      size: columnWidths.tgl_dibutuhkan,
      cell: ({ getValue, row }) => (
        <span className={`font-medium tabular-nums ${row.getIsSelected() ? 'text-green-600' : 'text-gray-600'}`}>
          {formatIndoDateStr(getValue() as string)}
        </span>
      )
    },
    ...
    },
    {
      ...
      cell: ({ getValue, row }) => <span className={`${row.getIsSelected() ? 'text-green-800' : 'text-gray-600'}`}>{getValue() as string}</span>
    },
    {
    ...
    },
    {
      ...
      cell: ({ getValue, row }) => {
        ...
        return (
          <span className={`font-black text-[10px] px-2 py-0.5 rounded-full border ${isClosed ? 'bg-slate-50 text-slate-400 border-slate-100' : 'bg-emerald-50 text-emerald-600 border-emerald-100'}`}>
            {val || 'OPEN'}
          </span>
        );
      }
    },
    {
      ...
      header: 'Ref. SPPH',
      size: columnWidths.faktur_spph,
      cell: ({ getValue, row }) => (
        <div className={`transition-colors truncate ${row.getIsSelected() ? 'text-green-600' : 'text-gray-400'}`} dangerouslySetInnerHTML={{ __html: getValue() as string || '-' }} />
      )
    },
    {
    ...
    },
    {
      ...
      cell: ({ getValue, row }) => (
        <span className={`truncate block ${row.getIsSelected() ? 'text-green-800' : 'text-gray-600'} italic text-[12px]`} title={getValue() as string || ''}>
          {getValue() as string || '–'}
        </span>
      )
    },
    {
      ...
      },
      {
        ...
        cell: ({ getValue, row }) => (
          <span className={`text-[11px] font-mono ${row.getIsSelected() ? 'text-green-600' : 'text-gray-300'}`}>
            {getValue() as string}
          </span>
        )
      }
            ...
            <div className="flex items-center gap-4">
              {selectedIds.size > 0 && (
                <div className="flex items-center gap-3 animate-in fade-in slide-in-from-right-2">
                  ...
                  <button onClick={clearSelection} className="text-[12px] font-black text-rose-500 hover:text-rose-600 underline underline-offset-4">Batal</button>
                </div>
              )}
              {loadTime !== null && (
