...
export default function PenerimaanPembelianClient() {
        ...
        if (active && mountedRef.current) {
          setData(prev => {
            const processData = (items: any[]) => (items || []).map((item: any) => {
              ...
              if (item.mydata) {
                try { 
                  parsedMyData = typeof item.mydata === 'string' ? JSON.parse(item.mydata) : item.mydata; 
                } catch(e){}
              }
  ...
  useEffect(() => {
    ...
    const savedPeriod = localStorage.getItem('PenerimaanPembelianClient_scrapedPeriod');
    if (savedPeriod) {
      try {
  ...
  const handleFetch = async () => {
    ...
    const processChunk = async (chunk: any) => {
      try {
        const res = await fetch(`/api/scrape-penerimaan-pembelian?start=${chunk.start}&end=${chunk.end}`);
        if (res.ok) {
          successCount++;
          const json = await res.json();
    ...
    try {
      ...
      if (successCount > 0) {
        ...
        localStorage.setItem('PenerimaanPembelianClient_scrapedPeriod', JSON.stringify(periodStr));
        setIsBatching(false);
      ...
      if (err.message?.includes('locked')) {
        setError('Database sedang sibuk. Silakan coba lagi.');
      } else {
  ...
  const columns: ColumnDef<any>[] = useMemo(() => [
    ...
    {
      accessorKey: 'top_hari',
      header: 'TOP',
      size: columnWidths.top_hari,
      cell: ({ getValue }) => <span className="tabular-nums font-bold text-slate-500">{String(getValue() || 0)} Hr</span>,
      meta: { align: 'right' }
    },
    {
        accessorKey: 'jthtmp',
    ...
    },
    {
      ...
      cell: ({ getValue, row }) => (
        <span className={`font-medium ${row.getIsSelected() ? 'text-green-700' : 'text-gray-500'}`}>
          {getValue() as string || '-'}
        </span>
      )
    },
    ...
    },
    {
        accessorKey: 'diskon',
        header: 'Diskon',
        size: columnWidths.diskon,
        meta: { align: 'right' },
        cell: ({ getValue, row }) => (
          <div className={`flex items-center justify-end font-medium tabular-nums transition-colors ${row.getIsSelected() ? 'text-rose-700' : 'text-gray-600'}`}>
            <span className={`text-[10px] font-bold mr-1 ${row.getIsSelected() ? 'text-rose-400' : 'text-gray-300'}`}>Rp</span>
            <span>{Number(getValue() || 0).toLocaleString('id-ID', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
          </div>
        )
      },
    {
      accessorKey: 'persppn',
      header: '% PPN',
      size: columnWidths.persppn,
      meta: { align: 'right' },
      cell: ({ getValue }) => <span className="text-gray-500 tabular-nums">{Number(getValue() || 0).toFixed(2)}%</span>
    },
    {
      accessorKey: 'ppn',
      header: 'PPN',
      size: columnWidths.ppn,
      meta: { align: 'right' },
      cell: ({ getValue, row }) => (
        <div className={`flex items-center justify-end font-medium tabular-nums transition-colors ${row.getIsSelected() ? 'text-green-700' : 'text-gray-700'}`}>
          <span className={`text-[10px] font-bold mr-1 ${row.getIsSelected() ? 'text-green-400' : 'text-gray-300'}`}>Rp</span>
    ...
    },
    {
      ...
      meta: { align: 'right' },
      cell: ({ getValue, row }) => (
        <div className={`flex items-center justify-end font-extrabold tabular-nums transition-colors ${row.getIsSelected() ? 'text-green-800' : 'text-gray-800'}`}>
          <span className={`text-[10px] font-bold mr-1 ${row.getIsSelected() ? 'text-green-400' : 'text-gray-300'}`}>Rp</span>
          <span>{Number(getValue() || 0).toLocaleString('id-ID', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
        </div>
      )
    },
    {
        accessorKey: 'hutang',
        header: 'Hutang',
        size: columnWidths.hutang,
        meta: { align: 'right' },
        cell: ({ getValue, row }) => (
          <div className={`flex items-center justify-end font-bold tabular-nums transition-colors ${row.getIsSelected() ? 'text-rose-700' : 'text-rose-500'}`}>
            <span className={`text-[10px] font-bold mr-1 opacity-50`}>Rp</span>
            <span>{Number(getValue() || 0).toLocaleString('id-ID', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
          </div>
        )
      },
      {
        accessorKey: 'kas',
        header: 'Bayar Kas',
        size: columnWidths.kas,
        meta: { align: 'right' },
        cell: ({ getValue, row }) => (
          <div className={`flex items-center justify-end font-bold tabular-nums transition-colors ${row.getIsSelected() ? 'text-green-700' : 'text-green-600'}`}>
            <span className={`text-[10px] font-bold mr-1 opacity-50`}>Rp</span>
            <span>{Number(getValue() || 0).toLocaleString('id-ID', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
          </div>
        )
    ...
    {
        accessorKey: 'tgl_lunas',
        header: 'Tgl Lunas',
        size: columnWidths.tgl_lunas,
        cell: ({ getValue, row }) => <span className={`text-[11px] font-bold font-mono ${row.getIsSelected() ? 'text-green-700' : 'text-gray-400'}`}>{String(getValue() || '-')}</span>
    },
    {
      accessorKey: 'keterangan_pr',
      header: 'Keterangan PB',
      size: columnWidths.keterangan_pr,
      cell: ({ getValue }) => <span className="text-gray-500 italic text-[12px] whitespace-nowrap">{getValue() as string || '-'}</span>
    },
    {
    ...
    },
    {
      ...
      cell: ({ getValue }) => <span className="text-[11px] text-gray-300 font-mono">{String(getValue() || '')}</span>
    }
  ], [columnWidths]);
