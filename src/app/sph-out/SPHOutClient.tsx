...
export default function SphOutClient() {
  ...
  const handleFetch = async () => {
    ...
    const processChunk = async (chunk: any) => {
      try {
        const res = await fetch(`/api/scrape-sph-out?start=${chunk.start}&end=${chunk.end}&metaStart=${startStr}&metaEnd=${endStr}`);
        if (res.ok) {
          successCount++;
          const json = await res.json();
    ...
    try {
      ...
      if (successCount > 0) {
        const periodStr = persistScraperPeriod({ stateKey: 'sphOutState', periodKey: 'SphOutClient_scrapedPeriod' }, startDate, endDate);
  ...
  const columns: ColumnDef<any>[] = useMemo(() => [
    ...
    },
    {
      ...
      cell: ({ getValue }) => <span className="text-gray-500">{getValue() as string || '0'}</span>
    },
    {
      accessorKey: 'faktur_bom',
      header: 'Faktur BOM',
      size: columnWidths.faktur_bom,
      cell: ({ getValue, row }) => (
        <span className={`font-medium truncate transition-colors ${row.getIsSelected() ? 'text-green-600' : 'text-gray-400'}`}>
          {getValue() as string || '-'}
        </span>
      )
    },
    {
      ...
      cell: ({ getValue, row }) => (
        <span className={`font-medium transition-colors ${row.getIsSelected() ? 'text-green-600' : 'text-gray-600'}`}>
          {getValue() as string || '-'}
        </span>
      )
    },
    ...
    },
    {
      ...
      meta: { align: 'right' },
      cell: ({ getValue }) => <span className="text-gray-500">{Number(getValue() || 0).toLocaleString('id-ID', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
    },
    {
      accessorKey: 'ppn',
      header: 'PPN',
    ...
    },
    {
      ...
      meta: { align: 'right' },
      cell: ({ getValue, row }) => (
        <div className={`flex items-center justify-end font-medium tabular-nums transition-colors ${row.getIsSelected() ? 'text-green-700' : 'text-gray-800'}`}>
          <span className={`text-[10px] font-bold mr-1 ${row.getIsSelected() ? 'text-green-400' : 'text-gray-300'}`}>Rp</span>
          <span>{Number(getValue() || 0).toLocaleString('id-ID', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
        </div>
      )
    ...
    },
    {
      accessorKey: 'barang',
      header: 'Barang',
      size: columnWidths.barang,
      cell: ({ getValue, row }) => (
        <span className={`truncate block transition-colors ${row.getIsSelected() ? 'text-green-800' : 'text-gray-600'}`} title={getValue() as string || ''}>
          {getValue() as string || '–'}
        </span>
      )
    },
      ...
      <div className="flex-1 flex flex-col gap-4 overflow-hidden min-h-0 relative">
        <div className="flex flex-col gap-4 shrink-0">
          ...
          <div className="relative w-full group">
            ...
            <input
              type="text"
              placeholder="Cari faktur, pelanggan, keterangan..."
