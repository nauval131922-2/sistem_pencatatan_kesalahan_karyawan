...
import { Search, ChevronLeft, ChevronRight, Package, Calendar, User, Tag, Hash, RefreshCw, BarChart3, Download, Printer, Loader2, AlertCircle, Clock, ArrowUp, ArrowDown, ArrowUpDown, FileText, FileCheck, ShieldCheck } from 'lucide-react';
...
export default function SPHOutClient() {
  ...
  // Note: Date persistence moved to handleFetch to follow "Scrape-First" requirement
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        const res = await fetch(`/api/sph-out?page=${page}&limit=${PAGE_SIZE}&search=${encodeURIComponent(debouncedQuery)}&from=${formatDateToYYYYMMDD(startDate)}&to=${formatDateToYYYYMMDD(endDate)}&_t=${Date.now()}`);
        if (!active) return;

        if (res.ok) {
          const json = await res.json();
          if (!json.success) throw new Error(json.error || 'Terjadi kesalahan saat memuat data');
  ...
  const handleFetch = async () => {
    ...
    const processChunk = async (chunk: any) => {
      try {
        const res = await fetch(`/api/scrape-sph-out?start=${chunk.start}&end=${chunk.end}`);
        if (res.ok) {
          successCount++;
          const json = await res.json();
    ...
    try {
      ...
      if (successCount > 0) {
        ...
        setDialog({
          ...
          message: failCount > 0 
            ? `Berhasil menarik ${totalScraped} data SPH dari Digit. (${failCount} bulan gagal)`
            : `Berhasil menarik ${totalScraped} data SPH dari Digit.`
        });
        ...
        setError("Gagal menarik data. Periksa koneksi.");
      }
    } catch (err: any) {
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 relative z-10">
          ...
          <div className="shrink-0 flex items-center gap-3">
            ...
            <button
              ...
              className="px-5 h-10 bg-green-600 hover:bg-green-700 text-white text-[13px] font-extrabold rounded-lg transition-all flex items-center justify-center gap-2.5 disabled:opacity-70 shadow-sm active:scale-[0.98]"
            >
              {isBatching ? <Loader2 size={16} className="animate-spin" /> : <RefreshCw size={16} className={loading && data === null ? "animate-spin" : ""} />}
            ...
            <div className="flex-1 flex flex-col min-h-0 gap-2 overflow-hidden">
              <div className="bg-white border border-gray-200 shadow-sm rounded-[10px] overflow-hidden flex-1 flex flex-col min-h-0 relative">
                <div className="overflow-auto custom-scrollbar flex-1 min-h-0" onScroll={handleScroll}>
                  <table className="text-left relative border-collapse table-fixed" style={{ width: totalTableWidth, minWidth: '100%' }}>
                    <thead className="sticky top-0 z-20 bg-white shadow-sm">
                      <tr>
                        ...
                        <th className="px-6 py-3 border-b border-r border-gray-200 relative group cursor-pointer hover:bg-gray-50 transition-colors whitespace-nowrap overflow-hidden" style={{ width: columnWidths.barang }} onClick={() => toggleSort('barang')}>
                          <div className="flex items-center gap-2 text-[12px] leading-none text-[#6b7280] font-bold tracking-wider overflow-hidden">
                            <span className="truncate">Barang</span>
                            <SortIcon config={sortConfig} sortKey="barang" />
                          </div>
                          <div className="absolute -right-2 top-0 bottom-0 w-4 z-30 cursor-col-resize group/resizer" onMouseDown={(e) => startResizing('barang', e)} onClick={(e) => e.stopPropagation()}>
                            <div className="absolute inset-y-0 right-2 w-[2px] bg-transparent group-hover/resizer:bg-green-500/50 group-active/resizer:bg-green-600 transition-colors" />
                          </div>
                        </th>
                        ...
                        <th className="px-6 py-3 border-b border-r border-gray-200 relative group cursor-pointer hover:bg-gray-50 transition-colors whitespace-nowrap overflow-hidden" style={{ width: columnWidths.total }} onClick={() => toggleSort('total')}>
                          <div className="flex items-center gap-2 text-[12px] leading-none text-[#6b7280] font-bold tracking-wider overflow-hidden">
                            ...
                            <SortIcon config={sortConfig} sortKey="total" />
                          </div>
                        ...
                        <th className="px-6 py-3 border-b border-gray-200 relative group cursor-pointer hover:bg-gray-50 transition-colors whitespace-nowrap overflow-hidden" style={{ width: columnWidths.faktur_so }} onClick={() => toggleSort('faktur_so')}>
                          <div className="flex items-center gap-2 text-[12px] leading-none text-[#6b7280] font-bold tracking-wider overflow-hidden">
                            <span className="truncate">Faktur SO</span>
                            <SortIcon config={sortConfig} sortKey="faktur_so" />
                          </div>
                          <div className="absolute -right-2 top-0 bottom-0 w-4 z-30 cursor-col-resize group/resizer" onMouseDown={(e) => startResizing('faktur_so', e)} onClick={(e) => e.stopPropagation()}>
                            <div className="absolute inset-y-0 right-2 w-[2px] bg-transparent group-hover/resizer:bg-green-500/50 group-active/resizer:bg-green-600 transition-colors" />
                          </div>
                        </th>
                      </tr>
                    ...
                    <tbody className="">
                      {sortedData.map((row, idx) => {
                        ...
                        <tr 
                          ...
                          className={`transition-all duration-150 group h-9 cursor-pointer select-none border-b border-gray-200 hover:bg-green-50/40 ${
                            isSelected ? 'bg-green-50 shadow-[inset_4px_0_0_0_#16a34a]' : idx % 2 === 1 ? 'bg-slate-50/40' : 'bg-white'
                          }`}
                        >
                          <td className="p-0 border-b border-gray-200 transition-colors whitespace-nowrap overflow-hidden" title={row.faktur}>
                            <div className={`px-6 py-1 border-r border-gray-200 h-fit flex items-center text-[12px] leading-none font-bold truncate ${isSelected ? 'text-green-600' : 'text-gray-700 group-hover:text-gray-500'}`}>{row.faktur}</div>
                          </td>
                          <td className="p-0 border-b border-gray-200 transition-colors whitespace-nowrap overflow-hidden" title={row.barang}>
                            <div className={`px-6 py-1 border-r border-gray-200 h-fit flex items-center text-[12px] leading-none font-bold truncate ${isSelected ? 'text-green-800' : 'text-gray-700 group-hover:text-gray-900'}`}>{row.barang}</div>
                          </td>
                          <td className="p-0 border-b border-gray-200 transition-colors whitespace-nowrap overflow-hidden" title={row.kd_pelanggan || ''}>
                            <div className={`px-6 py-1 border-r border-gray-200 h-fit flex items-center text-[12px] leading-none font-medium ${isSelected ? 'text-green-700' : 'text-gray-500'}`}>
                              <div className={`inline-block px-2.5 py-1 rounded-md transition-colors truncate whitespace-nowrap max-w-full leading-none ${isSelected ? 'bg-green-100 text-green-700' : 'bg-slate-100/60 text-gray-500 border border-gray-200/50 group-hover:bg-white'}`}>{row.kd_pelanggan}</div>
                            </div>
                          </td>
                          <td className="p-0 border-b border-gray-200 transition-colors whitespace-nowrap overflow-hidden">
                            <div className={`px-6 py-1 border-r border-gray-200 h-fit flex items-center text-[12px] leading-none font-bold ${isSelected ? 'text-green-700' : 'text-gray-700'}`}>{formatIndoDateStr(row.tgl)}</div>
                          </td>
                          <td className="p-0 border-b border-gray-200 transition-colors whitespace-nowrap overflow-hidden">
                             <div className={`px-6 py-1 border-r border-gray-200 h-fit flex items-center font-extrabold tabular-nums text-[12px] leading-none ${isSelected ? 'text-green-700' : 'text-gray-800'}`}>
                               <div className="flex items-baseline justify-between gap-2 overflow-hidden w-full leading-none">
                                <span className={`text-[10px] font-bold shrink-0 ${isSelected ? 'text-green-400' : 'text-gray-300'}`}>Rp</span>
                                <span className="truncate">{Number(row.total).toLocaleString('id-ID', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                               </div>
                             </div>
                          </td>
                          <td className="p-0 border-b border-gray-200 transition-colors whitespace-nowrap overflow-hidden">
                             <div className="px-6 py-1 border-r border-gray-200 h-fit flex items-center justify-center leading-none">
                               <span className={`px-2 py-1 rounded-full text-[11px] leading-none font-bold uppercase transition-colors ${
                                 row.status === 'Aktif' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'
                               }`}>
                                 {row.status}
                               </span>
                             </div>
                          </td>
                          <td className={`px-6 border-b border-gray-200 transition-colors whitespace-nowrap overflow-hidden leading-none ${isSelected ? 'text-green-600' : ''}`} title={row.faktur_so || 'Belum proses SO'}>
                            {row.faktur_so ? (
                              <div className="flex items-center gap-1.5 font-bold text-[12px] leading-none truncate">
                                  <FileCheck size={14} className="shrink-0" />
                                  <span className="truncate leading-none">{row.faktur_so}</span>
                              </div>
                            ) : (
                              <div className={`flex items-center gap-1.5 italic font-medium text-[12px] leading-none truncate ${isSelected ? 'text-green-400/60' : 'text-gray-300'}`}>
                                  <FileText size={14} className="opacity-40 shrink-0" />
                                  <span className="truncate leading-none">Belum proses SO</span>
                              </div>
                            )}
                          </td>
                        </tr>
                  ...
                  {data && data.length < totalCount && (
                    <div className="p-4 flex justify-center border-t border-gray-200 bg-gray-50/30">
              ...
              <div className="flex items-center justify-between shrink-0 px-1 mt-1">
                <span className="text-[12px] leading-none font-bold text-gray-400">
                  {totalCount === 0 ? 'Tidak ada data SPH' : `Menampilkan ${data?.length || 0} dari ${totalCount} data SPH`}
                </span>
                <div className="flex items-center gap-4">
                  {selectedIds.size > 0 && (
