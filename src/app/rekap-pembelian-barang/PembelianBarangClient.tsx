...
export default function PembelianBarangClient() {
  ...
  useEffect(() => {
    ...
    const savedPeriod = localStorage.getItem('PembelianBarangClient_scrapedPeriod');
    if (savedPeriod) {
      try {
    ...
    const saved = localStorage.getItem('rbpState');
    if (saved) {
      try {
  ...
  const handleFetch = async () => {
    ...
    try {
      ...
      if (successCount > 0) {
        ...
        localStorage.setItem('PembelianBarangClient_scrapedPeriod', JSON.stringify(periodStr));
