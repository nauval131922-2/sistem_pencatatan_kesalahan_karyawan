...
export default function PelunasanHutangClient() {
  ...
  const handleFetch = async () => {
    ...
    const processChunk = async (chunk: any) => {
      try {
        const res = await fetch(`/api/scrape-pelunasan-hutang?start=${chunk.start}&end=${chunk.end}&metaStart=${startStr}&metaEnd=${endStr}`);
        if (res.ok) {
          successCount++;
          const json = await res.json();
    ...
    try {
      ...
      if (successCount > 0) {
        const periodStr = persistScraperPeriod({ stateKey: 'phState', periodKey: 'PelunasanHutangClient_scrapedPeriod' }, startDate, endDate);
