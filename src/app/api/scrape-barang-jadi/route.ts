...
export async function GET(request: NextRequest) {
  try {
    ...
    const fetchDay = async (date: Date): Promise<Record<string, any>[]> => {
      ...
      // API Endpoint for Barang Jadi (Berdasarkan script python)
    ...
    const insertStmt = db.prepare(`
      INSERT INTO barang_jadi (tgl, nama_barang, qty, satuan, nama_prd, hp, raw_data)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
    ...
    db.transaction(() => {
      for (const record of finalRecords) {
        ...
        try {
          ...
          console.error('Failed to import barang_jadi:', record.nama_barang, err);
        }
      }
    })();
